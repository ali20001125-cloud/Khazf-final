...
const sections = [
  ...
  { href: "/admin/inventory/", label: "المخزون", badge: "lowStock" as const },
...
export default async function AdminHome() {
  ...
  const cards = [
    { Icon: Clock, l: "بانتظار التوصيل", v: fmt(s.pending), warn: s.pending > 0, href: "/admin/orders/" },
    { Icon: ShoppingCart, l: "طلبات اليوم", v: fmt(s.todayOrders), sub: `${fmt(s.salesToday)} د.ع`, href: "/admin/orders/" },
    { Icon: Users, l: "عملاء جدد اليوم", v: fmt(s.newCustomersToday), href: "/admin/customers/" },
    { Icon: TrendingUp, l: "زوّار اليوم", v: fmt(s.viewsToday), sub: s.conv > 0 ? `تحويل ${s.conv.toFixed(1)}٪` : undefined, href: "/admin/analytics/" },
    { Icon: Wallet, l: "مبيعات الأسبوع", v: `${fmt(s.salesWeek)} د.ع`, href: "/admin/orders/" },
    { Icon: AlertTriangle, l: "مخزون منخفض", v: fmt(s.lowStock), warn: s.lowStock > 0, href: "/admin/inventory/" },
    { Icon: Star, l: "تقييمات بالانتظار", v: fmt(s.pendingReviews), warn: s.pendingReviews > 0, href: "/admin/reviews/" },
    { Icon: ShoppingCart, l: "سلات مهجورة (٢٤س)", v: fmt(s.abandonedActive), warn: s.abandonedActive > 0, href: "/admin/analytics/" },
  ];
      ...
      {s.recentPending.length > 0 && (
        <div className="mt-6">
          <div className="mb-3 flex items-center justify-between">
            ...
            <Link href="/admin/orders/" className="text-[12px] font-semibold text-accent">كل الطلبات ←</Link>
          </div>
          <div className="space-y-2">
            {s.recentPending.map((o) => (
              <Link key={String(o.order_number)} href={`/admin/orders/?q=${o.order_number}`}
