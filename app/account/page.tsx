...
function AccountInner() {
  ...
  return (
    <div ref={scope} className="mx-auto max-w-2xl px-5 pb-24 pt-28 md:pt-32">
      ...
      <div className="reveal rounded-[6px] bg-olive p-7 text-olive-text">
      ...
      <div className="reveal mt-4 rounded-[6px] border border-line bg-card p-6">
      ...
      <Link href="/products/"
        className="reveal mt-4 flex items-center justify-between gap-3 rounded-[6px] border border-clay/25 bg-clay/6 p-5 transition-all active:scale-[0.99]">
      ...
      <div className="reveal mt-6 flex gap-2">
        {([["orders", "طلباتي", Package], ["cashback", "الكاش باك", Wallet], ["fav", "المفضلة", Heart]] as const).map(([k, label, Icon]) => (
          <button key={k} onClick={() => setTab(k)}
            className={`flex flex-1 flex-col items-center justify-center gap-1 rounded-[4px] py-2.5 text-[11.5px] font-bold transition-all ${
      ...
      <div className="reveal mt-4">
            ...
            <div className="space-y-2.5">
              {me.orders!.map((o) => {
                ...
                <div key={o.orderNumber}
                  className="rounded-[5px] border border-line bg-card px-5 py-4">
                  <Link href={`/invoice/?n=${o.orderNumber}&p=${me.phone}`}
                    className="flex items-center gap-3 transition-all active:scale-[0.99]">
                    <div className="h-11 w-11 shrink-0 overflow-hidden rounded-[4px] bg-bg-alt">
                      {img
        ...
        {tab === "cashback" && (
          <div className="rounded-[6px] border border-line bg-card p-7 text-center">
...
function EditProfile({ me, onClose }: { me: Me; onClose: () => void }) {
  ...
  const inp = "w-full rounded-[4px] border border-line bg-card px-4 py-3.5 text-[14px] outline-none focus:border-accent";
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-ink/40 backdrop-blur-sm md:items-center" onClick={onClose}>
      <div className="max-h-[92vh] w-full max-w-md overflow-y-auto rounded-t-[8px] bg-cream p-6 md:rounded-[8px]" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-5 space-y-3">
          ...
          {ok && <p className="rounded-[4px] bg-olive/10 px-4 py-2.5 text-center text-[12.5px] font-bold text-olive">حُفظت بياناتك ✓</p>}

          <div className="flex gap-2.5 pt-1">
            <button onClick={onClose} className="flex-1 rounded-[4px] border border-line bg-card py-3.5 text-[14px] font-bold text-muted active:scale-[0.98]">
              إلغاء
            </button>
            <button onClick={save} disabled={busy || ok}
              className="flex-[1.5] rounded-[4px] bg-olive py-3.5 text-[14px] font-bold text-olive-text active:scale-[0.98] disabled:opacity-60">
...
function Empty({ icon: Icon, text, cta }: { icon: typeof Package; text: string; cta?: boolean }) {
  return (
    <div className="rounded-[6px] border border-dashed border-line bg-card p-10 text-center">
...
function GoogleBtn({ label = "المتابعة عبر Google" }: { label?: string }) {
  return (
    <button onClick={startGoogle}
      className="flex w-full items-center justify-center gap-3 rounded-[5px] border border-line bg-card py-4 text-[14.5px] font-bold transition-all hover:-translate-y-0.5 hover:shadow-sm active:scale-[0.98]">
  ...
  return (
    <div ref={scope} className="mx-auto max-w-sm px-6 pb-24 pt-28">
      ...
      <button onClick={google} disabled={busy}
        className="reveal mt-7 flex w-full items-center justify-center gap-3 rounded-[5px] border border-line bg-card py-4 text-[14.5px] font-bold transition-all hover:-translate-y-0.5 active:scale-[0.98] disabled:opacity-60">
        <svg width="19" height="19" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>
        المتابعة عبر Google
      </button>
      ...
      {step === "email" ? (
        <div className="reveal space-y-3">
          ...
          {suggest && (
            <div className="rounded-[4px] border border-gold/50 bg-gold/8 px-4 py-3 text-center">
...
function ConnectGoogleHint() {
  return (
    <div className="reveal mt-6 rounded-[5px] border border-dashed border-line bg-card p-5 text-center">
