"use client";
/** نموذج القهوة — حقول القهوة فقط (بلا أدوات) */
...
export default function CoffeeForm({
  p = {}, places, chosenPlaces = [], action, submitLabel,
}: {
  p?: P; places: Place[]; chosenPlaces?: number[];
  action: (f: FormData) => Promise<void>; submitLabel: string;
}) {
  return (
    <form action={action} className="space-y-4">
      {p.id && <input type="hidden" name="id" value={p.id} />}
      <input type="hidden" name="type" value="COFFEE" />
      ...
      <Card className="grid gap-4 p-5 sm:grid-cols-2">
        ...
        <div className="sm:col-span-2">
          <Field label="صور المحصول" hint="ارفع عدة صور — الأولى الرئيسية"><MultiImageUpload name="images" initial={p.images ?? []} /></Field>
        </div>
      ...
      <Card className="grid gap-4 p-5">
        <Field label="الوصف (يظهر بصفحة المحصول)"><textarea name="description" rows={2} defaultValue={p.description ?? ""} className={inputCls} /></Field>
      ...
      <Card className="grid gap-4 p-5 sm:grid-cols-3">
        ...
        <Field label="تاريخ التحميص" hint="يظهر بصفحة المنتج ٣٠ يوماً ثم يختفي تلقائياً">
          ...
          <textarea name="brewMethods" rows={2}
            defaultValue={(p.brewMethods ?? []).join("، ")}
            placeholder="إسبريسو، V60، كيمكس، فرنش برس، لاتيه وكابتشينو، آيس لاتيه، كولد برو، قهوة تركية"
            className={`${inputCls} mt-2`} />
          <p className="mt-1 text-[11px] text-muted">المشروبات التي تناسبها — تظهر في معرض الصور · افصل بفاصلة</p>
      ...
      <details className="group rounded-[18px] border border-line bg-card">
        <summary className="cursor-pointer list-none px-5 py-4 text-[13px] font-bold text-muted">
          إعدادات متقدمة (المعرّف · Badge · مخزون · نكهة · قصة) <span className="ms-1 inline-block transition-transform group-open:rotate-90">‹</span>
        </summary>
        <div className="grid gap-4 border-t border-line p-5 sm:grid-cols-3">
          ...
          <Field label="عتبة التنبيه (كيس)" hint="تحتها يحذّرك النظام">
            <input name="stockThresholdBags" defaultValue={p.stockThreshold != null ? Math.round(p.stockThreshold / 250) : 8} className={`${inputCls} font-num`} dir="ltr" />
          </Field>
