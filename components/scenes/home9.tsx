  ...
  return (
    <section className="border-b border-line">
      <div className="mx-auto max-w-4xl px-5 py-12 md:px-8 md:py-16">
        ...
        <Link href="/products/?cat=coffee"
          className="group mt-5 flex items-center justify-between border-t border-line pt-5 transition-colors hover:text-accent">
          <span className="text-[14px] font-bold">عرض كل المحاصيل والأدوات</span>
            ...
            <div className="flex -space-x-3 rtl:space-x-reverse">
              {shots.map((c) => (
                <div key={c.slug} className="h-14 w-14 overflow-hidden border-2 border-olive bg-bg-alt" style={{ borderRadius: 4 }}>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={c.image!} alt="" width={600} height={600} loading="lazy" decoding="async" className="h-full w-full object-cover" />
                </div>
              ))}
            </div>
          )}
...
/* ═══ ٥) أكمِل تجربتك — ثلاثة نماذج للاختيار ═══ */
